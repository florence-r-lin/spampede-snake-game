...
class SpampedeDisplay {
	...
	public void updateGraphics() {
		...
		int horizontalShift = (this.width - Preferences.NUM_CELLS_WIDE * cellSize)  / 2;
		int verticalShift = (Preferences.TITLE_Y + cellSize);
		for(int x = 0; x < Preferences.NUM_CELLS_WIDE; x++){
			for(int y = 0; y < Preferences.NUM_CELLS_TALL; y++){
				drawSquare(x * cellSize + horizontalShift , y * cellSize + verticalShift, theData.getCellColor(y, x));
			}
		}
