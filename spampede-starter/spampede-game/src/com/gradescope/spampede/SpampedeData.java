...
class SpampedeData {
	...
	public BoardCell getNextCellInDir() {
		if (this.currentMode.equals(SnakeMode.GOING_WEST)){
			return this.getWestNeighbor();
		}
		else if (this.currentMode.equals(SnakeMode.GOING_EAST)){
			return this.getEastNeighbor();
		}
		else if (this.currentMode.equals(SnakeMode.GOING_SOUTH)){
			return this.getSouthNeighbor();
		}
		else return this.getNorthNeighbor();
	}
	...
	*/
	public BoardCell getNextCellFromBFS() {
    	...
    	while(cellsToSearch.size() != 0){
			BoardCell center = cellsToSearch.remove();
			BoardCell[] neighbors = this.getNeighbors(center);
			for(int i = 0; i < neighbors.length; i++){
				if(neighbors[i].isOpen() && !neighbors[i].inSearchListAlready()){
					neighbors[i].setAddedToSearchList();
					cellsToSearch.add(neighbors[i]);
					neighbors[i].setParent(center);
					if(neighbors[i].isSpam()){
						return getFirstCellInPath(neighbors[i]);
					}
				}
			}
  	...
  	private BoardCell getFirstCellInPath(BoardCell start) {
		BoardCell snakeHead = this.getSnakeHead();
		while(start.getParent() != snakeHead && start != null){
   ...
   */
  	public void reverseSnake() {
		...
		//create new reversed snake
		LinkedList<BoardCell> snakeReverse = new LinkedList<BoardCell>();
		//iterate through current snake to create reversed snake
		for (int i = snakeCells.size() - 1; i >= 0; i--){
			BoardCell cell = snakeCells.get(i);
			if (i == snakeCells.size() - 1) {
				cell.becomeHead(); 
			} else {
				cell.becomeBody();
			}
			snakeReverse.add(cell);
		}
		snakeCells = snakeReverse;

	 	if (this.getNorthNeighbor() == this.getSnakeNeck()){
			this.setDirectionSouth();
		} else if (this.getSouthNeighbor() == this.getSnakeNeck()){
			this.setDirectionNorth();
		} else if (this.getEastNeighbor() == this.getSnakeNeck()){
